"""
    This file contain utilities for comparing actual datasets generated by unitxt. Common use of these utilities is
    after creating new version, in order to make sure that the generated datasets (exampels) remains the same.
...
def get_all_artifacts_in_catalog_by_type(catalog_path, artifact_type):
    artifacts = []
    for root, dirs, files in os.walk(os.path.join(catalog_path, artifact_type)):
        for file in files:
            artifact_abs_path = os.path.join(root, file)
            artifact_relative_path = os.path.relpath(artifact_abs_path, catalog_path)
            artifacts.append(artifact_relative_path.replace('/', '.').replace('.json', ''))
    return artifacts
...
def create_recipes_params_file(results_dir, catalogs, compare_catalogs):
    ...
    for catalog_path in catalogs:
        ...
        for type in ['cards', 'augmentors', 'system_prompts', 'formats']:
            if type not in type2artifacts:
                type2artifacts[type] = []
            type2artifacts[type] += get_all_artifacts_in_catalog_by_type(catalog_path, artifact_type=type)
    ...
    for catalog_path in compare_catalogs:
        ...
        for type in ['cards', 'augmentors', 'system_prompts', 'formats']:
            ...
            compared_type2artifacts[type] += get_all_artifacts_in_catalog_by_type(catalog_path, artifact_type=art_type)

    compared_type2artifacts['system_prompts'] = [a.replace('instruction', 'system_prompt')
                                                 for a in compared_type2artifacts['system_prompts']]

    for type in ['cards', 'augmentors', 'system_prompts', 'formats']:
        type2artifacts[type] = [a for a in type2artifacts[type] if a in compared_type2artifacts[type]]
        type2artifacts[type].sort()

    recipes = []
    for i, card in enumerate(type2artifacts['cards']):
        if 'ansible' in card:
            continue
        print(f'{i}: card: {card}')
        try:
            card_obj = fetch(card)
            recipes.append({'card': card,
                             'template': random.choice([e.artifact_identifier for e in card_obj.templates.items]),
            ...
            print(f'Warning: failed to fetch {card}')

    with open(os.path.join(results_dir, recipes_params_file_name), 'w') as file:
        json.dump(recipes, file)


def generate_examples_for_configuration(card, template, system_prompt, format, num_demos, is_old_version):
    ...
    inputs = {'card': card, 'template': template, system_prompt_field: system_prompt, 'format': format,
              'num_demos': num_demos, 'demos_pool_size': DEMOS_POOL_SIZE,
...
def get_file_name(dir, card, template, system_prompt, format, num_demos, augmentor=None):
    return os.path.join(dir, f'{card}__{template}__{system_prompt}__{format}__{num_demos}')


def generate_examples_for_configuration_and_save_in_file(recipe, dir, compare_dir, results):
    card = recipe['card']
    ...
    try:
        ...
        if os.path.isfile(file_name):
            results[results_key] = {'status': 'skip', 'reason': f"File already exists. Skipping {file_name}"}
            return
        if compare_dir is not None:
            compare_file_name = get_file_name(compare_dir, card, template, system_prompt, format, num_demos)
            if not os.path.isfile(compare_file_name):
                results[results_key] =  {'status': 'skip', 'reason': "File doesn't exist for compare. Skipping {compare_file_name}"}
                return
        examples = generate_examples_for_configuration(
            card, template, system_prompt, format, num_demos, is_old_version=compare_dir is not None)
        ...
        results[results_key] =  {'status': 'failure', 'exception': e,
                                 'reproduce': 'NNNresults = {' +f'"{results_key}": '+' {}}NNN'
                                              f'generate_examples_for_configuration_and_save_in_file(' +
                                              f'recipe={recipe}'
...
def handle_recipe(recipe, dir, compare_dir, results):
    generate_examples_for_configuration_and_save_in_file(recipe, dir, compare_dir, results)
...
def print_status(results, dir):
    ...
    try:
        ...
        print(f'Done: {len([e for e in results.values() if "status" in e and e["status"] == "success"])}')
        print(f'Failed: {len([e for e in results.values() if "status" in e  and e["status"] == "failure"])}')
        print(f'Skipped: {len([e for e in results.values() if "status" in e  and e["status"] == "skip"])}')
        print(f'Pending: {len([e for e in results.values() if "status" not in e])}')
        with open(os.path.join(dir, 'log.txt'), 'w') as file:
            for i, (recipe_name, status) in enumerate(results.items()):
                file.write(f"{i}: {recipe_name} -- {status}\n".replace("\\\'", "'").replace("NNN", "\n"))
    finally:
        lock.release()
...
def create_examples_for_recipes_file(recipes_list_file, results_dir, compare_dir, max_recipes=None, max_workers=100):
    os.makedirs(results_dir, exist_ok=True)
    with open(recipes_list_file, 'r') as file:
        recipes = json.load(file)

    if max_recipes is not None:
        recipes = recipes[:max_recipes]

    results = {get_file_name(dir='', **recipe): {'recipe': recipe} for recipe in recipes}
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_recipes = {executor.submit(handle_recipe, recipe, results_dir, compare_dir, results): recipe
