...
def get_dirs_files_lists(dir_a, dir_b):
    ...
    for file_name in common_files:
        with open(os.path.join(dir_a, file_name)) as f_a, open(
            os.path.join(dir_b, file_name)
        ) as f_b:
...
def create_main_report(report_dir, only_in_a, only_in_b, common_files, diff_files):
    ...
    with open(main_report_path, "w") as report_file:
        ...
        for i, file_name in enumerate(
            sorted(diff_files), len(only_in_a) + len(only_in_b) + 1
        ):
            link = f'<a href="{get_diff_report_name(file_name)}">{file_name}</a>'
            report_file.write(
                f"<tr><td>{i}</td><td>{link}</td><td>Both (Different Content)</td></tr>"
            )
        report_file.write("</table></body></html>")
    logger.info(f"Main report generated: {main_report_path}")
...
def compare_dirs(dir_a, dir_b, report_dir):
    ...
    for i, file in enumerate(sorted(diff_files)):
        logger.info(f"{i}/{len(diff_files)}: creating report for {file}")
        generate_diff_html(
            dir_a,
            dir_b,
            os.path.join(dir_a, file),
            os.path.join(dir_b, file),
