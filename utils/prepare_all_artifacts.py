...
def main():
    ...
    prepare_file_to_changed_files = defaultdict(list)
    prepare_file_to_added_files = defaultdict(list)
            ...
            for current_catalog_file in current_catalog_files:
                if (
                    ...
                    if not os.path.exists(
                        ...
                        prepare_file_to_added_files[prepare_file].append(
                        ...
                        prepare_file_to_changed_files[prepare_file].append(
                            current_catalog_file
                        )
    ...
    if prepare_file_to_added_files:
        msg = "\n* Some catalog json files generated by preparation files but not committed! *\n"
        msg += f"\n The following {len(prepare_file_to_added_files)} prepare files generated catalog files that are not included in the catalog.\n -> To fix: add the products of these prepare files to the catalog.\n"

        for prepare_file in sorted(prepare_file_to_added_files):
            msg += f" - {prepare_file} missing products:"
            for file in prepare_file_to_added_files[prepare_file]:
                msg += f"\n    [] {file} (missing)"
        logger.critical(msg)

    if prepare_file_to_changed_files:
        msg = "\n* Some catalog json files changed by preparation files but changes were not committed! *\n"
        msg += f"\n The following {len(prepare_file_to_changed_files)} prepare files generated catalog files of different contents from what is included in the (original branch's) catalog.\n -> To fix: update the branch's catalog files by the products of these prepare files."

        for prepare_file in sorted(prepare_file_to_changed_files):
            msg += f" - {prepare_file} changed products:"
            for file in prepare_file_to_changed_files[prepare_file]:
                msg += f"\n    [] {file} (changed)"
        logger.critical(msg)
    ...
    if catalog_files_not_generated_by_any_prepare_file:
        msg = "\n* Some catalog json files changed by preparation files but changes were not committed! *\n"
        msg += f"\nThe following {len(catalog_files_not_generated_by_any_prepare_file)} branch's catalog files are not generated by any of the branch's prepare files.\n  -> To fix: remove them from the branch's catalog."

        msg += "The following files were not generated by any preparation files and should be deleted:"
