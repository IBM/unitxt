...
class TestMetricSignifDifference(unittest.TestCase):
    ...
    def _test_signif(self, expected_pvalues_list: list, same_distr=True, continuous=True):
        np.random.seed(4)
        ...
        pvals_twosided = tester.signif_pair_diff(samples_list=model_res, alternative='two-sided')
        for observed, expected in zip(pvals_twosided["pvalues"], expected_pvalues_list[0]):
            self.assertAlmostEqual(first=observed, second=expected)
        pvals_onesided = tester.signif_pair_diff(samples_list=model_res, alternative='less')
        for observed, expected in zip(pvals_onesided["pvalues"], expected_pvalues_list[1]):
            self.assertAlmostEqual(first=observed, second=expected)
        ...
        pvals_twosided = tester.signif_pair_diff(samples_list=model_res, alternative='two-sided', permute=True)
        for observed, expected in zip(pvals_twosided["pvalues"], expected_pvalues_list[2]):
            self.assertAlmostEqual(first=observed, second=expected)
        pvals_onesided = tester.signif_pair_diff(samples_list=model_res, alternative='less', permute=True)
        for observed, expected in zip(pvals_onesided["pvalues"], expected_pvalues_list[3]):
            self.assertAlmostEqual(first=observed, second=expected)
    ...
    def test_signif_mcnemar_binary(self):
        ...
        pvalue = tester.signif_pair_diff(samples_list=binary_same)
        self.assertAlmostEqual(first=pvalue["pvalues"][0], second=0.1670684814453125)

        binary_diff = self.gen_binary_data(same_distr=False, nmodels=tester.nmodels)
        pvalue = tester.signif_pair_diff(samples_list=binary_diff)
        self.assertAlmostEqual(first=pvalue["pvalues"][0], second=0.028959274291992188)
        ...
        pvalue_1x1 = tester.signif_pair_diff(samples_list=samples_list)
        # p-value is 1 meaning there is no difference since both have same values exactly
        self.assertAlmostEqual(first=pvalue_1x1["pvalues"][0], second=1.0)
        ...
        self.assertAlmostEqual(first=pvalue_1x1["pvalues"][0], second=1.5777218104420236e-30)

        # contingency table is 1x2
        samples_list = [np.ones(shape=100), np.repeat(a=[0, 1], repeats=[47, 53])]
        pvalue_1x2 = tester.signif_pair_diff(samples_list=samples_list)
        self.assertAlmostEqual(first=pvalue_1x2["pvalues"][0], second=1.4210854715202004e-14)

        # contingency table is 2x1
        samples_list = [np.repeat(a=[0, 1], repeats=[49, 51]), np.zeros(shape=100)]
        pvalue_2x1 = tester.signif_pair_diff(samples_list=samples_list)
        self.assertAlmostEqual(first=pvalue_2x1["pvalues"][0], second=8.881784197001252e-16)
        ...
        pvalue_2x2 = tester.signif_pair_diff(samples_list=samples_list)
        self.assertAlmostEqual(first=pvalue_2x2["pvalues"][0], second=0.001953125)