...
class TestMetricSignifDifference(unittest.TestCase):
    ...
    def test_sample_and_report_compatibility_with_tester(self):
        ...
        with self.assertRaises(
            AssertionError, msg="nmodels must equal len(model_names)"
        ):
            PairedDifferenceTest(nmodels=3, model_names=["llama", "flan-t5"])
        with self.assertRaises(AssertionError, msg="model_names must be unique"):
            PairedDifferenceTest(nmodels=3, model_names=["llama", "flan-t5", "llama"])
        ...
        for ii, tstr in enumerate(testers):
            with self.assertRaises(
                AssertionError, msg="duplicated metric names should not be compatible"
            ):
                tstr._check_valid_signif_report_list(
                    test_results_list=[signif_reports[ii][0]] * 2
                )
            for jj, samp_list in enumerate(samples):
                if ii != jj:
                    for samp in samp_list:
                        with self.assertRaises(
                            AssertionError,
                            msg="tester should not be compatible with a sample list not matched to it",
                        ):
                            tstr._check_valid_samples(samples_list=samp)
                    for srep in signif_reports[jj]:
                        with self.assertRaises(
                            AssertionError,
                            msg="tester should not be compatible with a significance report not generated by it",
                        ):
                            tstr._check_valid_signif_report(test_res=srep)
                    with self.assertRaises(
                        AssertionError,
                        msg="tester should not be compatible with a list of significance reports not generated by it",
                    ):
                        tstr._check_valid_signif_report_list(
                            test_results_list=signif_reports[jj]
                        )
                    with self.assertRaises(
                        AssertionError,
                        msg="tester should not be compatible when at least one significace report was not generated by it",
                    ):
