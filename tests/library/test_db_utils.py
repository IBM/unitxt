...
class TestRemoteDatabaseConnector(unittest.TestCase):
    ...
    def test_execute_query_max_retries_exceeded(self, mock_post):
        ...
        self.assertIsNone(result)
        self.assertEqual(mock_post.call_count, 4)  # 1 initial call + 3 retries
...
class TestLocalSQLiteConnector(unittest.TestCase):
    ...
    def test_init_success(self):
        with patch(
            ...
            self.assertIsInstance(connector.conn, sqlite3.Connection)
            self.assertIsInstance(connector.cursor, sqlite3.Cursor)

    def test_init_no_db_id(self):
        self.db_config["db_id"] = None
        with self.assertRaises(ValueError):
            LocalSQLiteConnector(self.db_config)
    ...
    #         self.assertIn("CREATE TABLE table1(tab1col1 TEXT, tab1col2 INTEGER)", schema_text)
    #         self.assertIn("CREATE TABLE table2(tab2col1 REAL, tab2col2 TEXT)", schema_text)
    #         self.assertNotIn("CREATE TABLE sequence", schema_text)
    ...
    # @patch(
    #     "unitxt.db_utils.LocalSQLiteConnector.download_database",
    #     side_effect=lambda x: None,
    # )
    # def test_download_database(self, mock_download):
    #     with tempfile.TemporaryDirectory() as temp_dir:
    #         os.environ["UNITXT_ARTIFACTORIES"] = temp_dir
    #         connector = LocalSQLiteConnector(self.db_config)
    #         connector.download_database("bird/dev")
    #         mock_download.assert_called_with("bird/dev")

    #         repo_dir = os.path.join(
    #             connector.databases_folder, "datasets", "premai-io--birdbench"
    #         )
    #         hfapi = HfApi()
    #         hfapi.snapshot_download.assert_called_with(
    ...
    def test_download_database_unsupported_db(self):
        with self.assertRaises(NotImplementedError):
            connector = LocalSQLiteConnector(self.db_config)
            connector.download_database("unknown_db")
