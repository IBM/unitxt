...
class TestMetrics(unittest.TestCase):
    ...
    def test_f1_macro_multilabel_with_ignore_classes(self):
        metric = F1MacroMultiLabel()
        for class_to_ignore in metric.classes_to_ignore:
            references = [[[class_to_ignore]]]
            predictions = [[class_to_ignore]]
            ...
            references = [[[class_to_ignore]]]
            predictions = [[class_to_ignore, "x", "y"]]
            outputs = apply_metric(metric=metric, predictions=predictions, references=references)
            self.assertTrue(isnan(outputs[0]["score"]["global"]["score"]))
            ...
            references = [[[class_to_ignore]], [["x"]], [["y"]], [[class_to_ignore]], [[class_to_ignore]]]
            predictions = [[class_to_ignore], ["x"], ["x"], [class_to_ignore], [class_to_ignore]]
            outputs = apply_metric(metric=metric, predictions=predictions, references=references)
            self.assertAlmostEqual(global_target, outputs[0]["score"]["global"]["score"])

    def test_f1_micro_multilabel_with_ignore_classes(self):
        metric = F1MicroMultiLabel()
        for class_to_ignore in metric.classes_to_ignore:
            references = [[[class_to_ignore]]]
            predictions = [["cat", "dog"]]
            ...
            references = [[[class_to_ignore]], [["sad"]]]
            predictions = [["dog", "fustrated"], ["sad"]]
