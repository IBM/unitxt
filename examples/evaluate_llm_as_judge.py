...
# It checks two llama3 based judges - one based on a 8b model and one on a 70b model on
# summarization dataset.
#
# The results indicate that the 8b model gives a higher score to a wrong prediction vs the correct
# prediction in 20% of the examples, and gives a truncated corrected prediction higher score than
# the correct prediction 35% of examples.  This means it is not so good as judge for this task.
#
# On the other hand the 70b model is better.  It always gives a higher score for the correct prediction,
# and in only 5% of the cases gives the the truncated prediction a higher score.
# Note that even the 70b, gives relatively low average score for correct predictions (0.395 +/ 0.17)
...
for metric_to_check in metrics_to_check:
    # The dataset used to evaluate the metrics based on it's gold answers
    ...
    # 2. wrong predictions (where the prediction is the gold answer of the previous question)
    ...
    # The score of correct predictions should be close to 1 with low std
    # The score of wrong predictions should be close to 0 with low std
