...
card = TaskCard(
    loader=LoadHF(
        ...
        splits=["test"],
        filtering_lambda="lambda x: x['instruction_type'] == 'DP' and x['qtype'] in ['FactChecking']",
    ),
    preprocess_steps=[
        SplitRandomMix(
            mix={
...
add_to_catalog(card, "cards.tablebench_fact_checking", overwrite=True)
...
test_card(card, strict=False, loader_limit=200, demos_pool_size=-1, num_demos=1)
add_to_catalog(card, "cards.tablebench_fact_checking.legacy", overwrite=True)
