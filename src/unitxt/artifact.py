...
def is_library_module(module_name):
    ...
    if any(pkg_dir in file_path for pkg_dir in ["site-packages", "dist-packages"]):
        # For editable installs, check if the actual file is outside site-packages
        site_packages = sysconfig.get_path("purelib")

        # If the module file is actually outside site-packages, it's likely editable
        if not file_path.startswith(site_packages):
            # Check if there's a corresponding .egg-link or .pth file
            import glob

            package_name = module_name.split(".")[0]
            egg_links = glob.glob(
                os.path.join(site_packages, f"{package_name}*.egg-link")
            )
            pth_files = glob.glob(os.path.join(site_packages, f"*{package_name}*.pth"))

            if egg_links or pth_files:
                return False  # It's an editable install, treat as non-library
