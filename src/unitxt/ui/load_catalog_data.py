...
def get_templates_from_dict(template_data, card):
    ...
    if template_data.get("__type__") == "templates_dict":
        for template_name, template in template_data["items"].items():
            if isinstance(template, str):
                register_existing_template(template, templates, templates_jsons)
            elif isinstance(template, dict):
                register_inline_template(template_name, template, card, templates, templates_jsons)

    elif template_data.get("__type__") == "templates_list":
        for i, template in enumerate(template_data["items"]):
            if isinstance(template, str):
                register_existing_template(template, templates, templates_jsons)
            elif isinstance(template, dict):
    ...
    elif not template_data.get("__type__"):
...
def register_inline_template(template_name, template, card, templates, templates_jsons):
    template_name_prefix = card.replace('cards.', f'templates.tmp.') + '.{template}'
...
def load_cards_data():
    def is_valid_data(data):
        if "task" not in data or isinstance(data["task"], dict) or "templates" not in data:
            return False
        return True
...
def get_catalog_items(items_type):
    ...
    if PRIVATE_DIR:
        try:
            ...
            print(f"Failed to get {items_type}': {e}")
