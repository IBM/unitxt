...
def is_typing_instance(object, type):
    """
    ...
    if hasattr(type, '__origin__'):
        ...
        if origin is list or origin is set:
            return all(is_typing_instance(element, type_args[0]) for element in object)
        elif origin is dict:
            return all(is_typing_instance(key, type_args[0]) and is_typing_instance(value, type_args[1]) for key, value in object.items())
        elif origin is tuple:
            return all(is_typing_instance(element, type_arg) for element, type_arg in zip(object, type_args))
    else:
