...
class VerifyDataClassificationPolicy(Artifact):
    """Checks if data classifications of a given artifact and data are compatible.
    ...
    Args:
       artifact (optional, Artifact): an artifact object for which data classification policy
            is checked based on its attribute.
       artifact_name (optional, str): an artifact name for which data classification policy
            is checked based on value retrieved from env.


    Examples:
        metric = Accuracy(data_classification_policy=["public"])
        check = VerifyDataClassificationPolicy(artifact=metric)

        template = SpanLabelingTemplate(data_classification_policy=["pii", "propriety"])
        check = VerifyDataClassificationPolicy(artifact=template)

        UNITXT_DATA_CLASSIFICATION_POLICY = {"metrics.accuracy": ["public"]}
        check = VerifyDataClassificationPolicy(artifact_name="metrics.accuracy")
    """
    ...
    def verify(self):
        if not self.artifact and not self.artifact_name:
            raise ValueError(
                "Either 'artifact' or 'artifact_name' must be set. However, "
                "both attributes are 'None'."
            )

        if self.artifact and not isinstance(self.artifact, Artifact):
            raise ValueError(
                f"'artifact' must be an 'Artifact' object, for example an instance "
                f"of the 'Metric' class. However, '{self.artifact}' was given instead."
            )

        if self.artifact_name:
            error_msg = (
                f"'artifact_name' must be a proper string name of an artifact to be fetched. "
                f"For example: 'metrics.accuracy' or 'templates.span_labeling.extraction.carry'. "
                f"However, '{self.artifact_name}' was given instead."
            )
            ...
            try:
                fetch_artifact(self.artifact_name)
    ...
    def process(self, instance: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.artifact_name:
            ...
            name = self.artifact.get_artifact_type()
            data_classification_policy = self.artifact.data_classification_policy

        if not data_classification_policy:
            get_logger().warning(
                f"'{name}' has 'data_classification_policy' set empty, thus "
                f"it cannot verify if possibly sensitive data is handled in a "
                f"proper way. Set the 'data_classification_policy' in order to "
                f"avoid a potential sending of undesired data to external service."
            )
            return instance
        ...
        if not instance_data_classification:
            get_logger().warning(
                ...
                f"{data_classification_policy}. This may lead to sending of "
                f"undesired data to external service. Set the 'data_classification_policy' "
        ...
        if not any(
            ...
            raise ValueError(
                ...
                f"'{data_classification_policy}'. To allow this set the environment "
                f"variable 'UNITXT_DATA_CLASSIFICATION_POLICY' or change the '{name}' "
                f"object to include '{instance_data_classification}'."
            )
